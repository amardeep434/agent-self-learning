...
PY_CLAUDE_EXEMPTIONS=(
    ...
    "scripts/coach-rules-eval.py|prose only, in SUGGESTION_OVERRIDES: the replacement remediation text for no-slash-commands tells the user where their own harness keeps project-level slash commands (Claude Code: .claude/commands/; Copilot CLI: custom prompt files). That is ADVICE ABOUT THE USER'S HARNESS printed into a signal, the same category as doctor.sh's warning text, not a path this file resolves, reads or writes. No code path in this file constructs a ~/.claude path -- the framework's own paths still come from lib/paths.py and every store read goes through lib/telemetry.py"
    "scripts/mirror-skills.py|MIRROR_ROOTS deliberately names each harness's OWN skills directory (~/.claude/skills, ~/.copilot/skills) because publishing there is the entire point of this script: a skill under <store>/learned-skills/ is discovered by NO harness, which is why 46 well-formed skills were invisible to every agent. Every upstream this project cites delivers the same way (SkillOpt-Sleep -> ~/.claude/skills/skillopt-sleep-learned/SKILL.md, GEPA -> .claude/skills/<repo>/SKILL.md, Coach -> ~/.agents/skills/<slug>/SKILL.md). Harness neutrality is preserved by construction, not by avoiding the literal: the roots are a SYMMETRIC table with one entry per harness and no default, each is used ONLY if its parent already exists (probe, never infer -- so no ~/.copilot is created on a machine without Copilot and vice versa), and the framework's own store still comes from lib/paths.py. Nothing here reads the store from a harness path; it only ever writes a copy outward. tests/test-claude-absent.sh remains the regression guard, and tests/test-mirror-skills.sh case F pins the probe"
