...
| Windows | CI has run once, red on both Windows jobs; fix round A addressed the causes found, but no re-run has been observed since | CI has run once, red on both Windows jobs; fix round A addressed the causes found, but no re-run has been observed since | Windows CI failure is per-OS (the whole matrix cell fails), not per-harness — both harness columns show the same Windows result. CI declares an `ubuntu-latest, macos-latest, windows-latest` × Python `3.9, 3.13` matrix (`.github/workflows/ci.yml`); Ubuntu passed both jobs on the one run so far. Do not treat Windows/macOS as passing until a subsequent run is green. |
| macOS | CI has run once, red on both macOS jobs; fix round A addressed the causes found, but no re-run has been observed since | CI has run once, red on both macOS jobs; fix round A addressed the causes found, but no re-run has been observed since | Same per-OS caveat as the Windows row above — macOS CI failure is not specific to either harness. |
