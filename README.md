...
| 5 | Integration + Polish (config, caching, install, health check) | Done for Claude Code + Copilot CLI; VS Code Copilot Chat adapter **not started; feasibility established and tracked** in `docs/superpowers/vscode-adapter-spike.md` — a declared peer deferred by the 2026-07-25 plan under "Out of Scope (subsequent plans)", i.e. owed later. VS Code ships a hook system that needs no extension of ours, and our existing transcript parser reads its stored transcripts unmodified; blocked on one human-run spike. For CI status run `gh run list --branch main` — no run id is recorded here, deliberately; see "Agent compatibility" above for the Windows skip caveat |
