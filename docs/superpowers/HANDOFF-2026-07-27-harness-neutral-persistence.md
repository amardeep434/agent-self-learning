...
| **D** | ~~Register the Claude Code hooks~~ — **DONE 2026-07-28**: installed from merged `main`, merged additively into `~/.claude/settings.json` (every pre-existing hook group preserved), health 29/0/0, doctor HEALTHY. Note this also enabled **VS Code Copilot Chat**, which reads that file by default — deliberate, and the adapter handles its transcript format. **Still open:** rotate the `gho_` token if these logs are shared — GitHub documents keychain-first with plaintext fallback, and `/logout` does not revoke. | **USER DECISION** | [§D](../../.superpowers/sdd/2026-07-25-harness-neutral-persistence/final-sweep-findings.md#d-user-decision) |
...
- ~~**Route B (Coach export) has never run here**~~ — **CLOSED 2026-07-28.** The fork's
  `.vsix` was installed and the export run for the first time on any machine:
  `~/.aiec/summary-latest.json`, `schemaVersion: 1`, 32,929 occurrences over 555 sessions,
  and `coach-signals.py` produced 10 signals with `source=export` whose counts match the
  export exactly. A sanitized copy is vendored at `tests/fixtures/coach-export-v1.json`
  with a dedicated suite, so the route no longer rests on one manual run. Residual: that is
  **one export, from one day, from one install** — a `schemaVersion: 2` would fail loudly
  (by design) but is untested.
